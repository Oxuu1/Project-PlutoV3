...
{
    internal class FortniteV2
    {
        public static void LaunchSinglePlayer(string path, string username)
        {
            ...
            for (; ; )
            {
                ...
                if (flag)
                {
                    string Framework = Directory.GetCurrentDirectory() + "\\Framework.dll";
                    Thread.Sleep(3000);
                    PlutoInjector.Inject(id, Framework);
                    string console = Directory.GetCurrentDirectory() + "\\Rift.dll";
                    Thread.Sleep(9000);
